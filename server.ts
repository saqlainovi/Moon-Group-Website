...
`
  ...
  app.get('/api/cms/properties', async (req, res) => {
    ...
    // Ensure all properties have mandatory string fields
    const sanitizedProps = (db.properties || []).map((p: any) => {
      const def = defaultProperties.find((dp: any) => dp.id === p.id);
      return {
        ...(def || {}),
        ...p,
  ...
  app.post('/api/cms/properties', async (req, res) => {
    ...
    const def = defaultProperties.find((dp: any) => dp.id === prop.id);
    ...
    const idx = db.properties.findIndex((p: any) => p.id === normalizedProp.id);
    if (idx >= 0) {

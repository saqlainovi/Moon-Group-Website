...
export default function AdminCMS({ onBackToSite }: AdminCMSProps) {
  ...
  // CMS Content States
  const [activeTab, setActiveTab] = useState<'properties' | 'slides' | 'about' | 'concerns' | 'testimonials' | 'bookings' | 'partnerships' | 'settings'>('properties');
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (pin === 'moon2026' || pin === 'admin') {
      ...
      setAuthError('Invalid Admin passcode. Hint: moon2026');
    }
  };
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        try {
          ...
          triggerStatus('success', 'Property deleted from Firestore.');
          loadCMSContent();
        } catch (error) {
          triggerStatus('error', 'Failed to delete property.');
        }
      }
    });
  };
  ...
  const handleSaveSlide = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isQuota) {
        ...
        triggerStatus('error', 'Error writing hero slide.');
      }
    }
  };
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        try {
          ...
          triggerStatus('success', 'Slide deleted.');
          loadCMSContent();
        } catch (error) {
          triggerStatus('error', 'Delete slide failed.');
        }
      }
    });
  };
  ...
  const handleSaveConcern = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isQuota) {
        ...
        triggerStatus('error', 'Error saving Concern.');
      }
    }
  };
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        try {
          ...
          triggerStatus('error', 'Removal failed.');
        }
      }
    });
  };
  ...
  const handleSaveTestimonial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isQuota) {
        ...
        triggerStatus('error', 'Failed to save customer review.');
      }
    }
  };
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isQuota) {
        ...
        triggerStatus('error', 'Failed to save global site settings.');
      }
    }
  };
      ...
      <header className="sticky top-0 z-40 bg-[#161617] border-b border-white/10 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="block font-mono text-[9px] text-[#6B6B6A] uppercase tracking-widest">Live Cloud Firestore Synced</span>
          </div>
        </div>
                  ...
                  <div className="p-6 border border-white/10 bg-black/10 rounded-sm space-y-6">
                    ...
                    <div className="space-y-4 p-4 border border-white/5 bg-black/40 rounded-sm">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 border-t border-white/5 pt-3">
                        ...
                        <div className="flex-1 space-y-1">
                          <span className="block text-[9px] uppercase text-slate-500">Or Upload Multiple Image Files</span>
                          <input 
                            type="file" 
                            accept="image/*"
                            multiple
                            onChange={async (e) => {
                              ...
                              if (files.length > 0) {
                                 ...
                                 const newBase64s = await Promise.all(files.map(f => resizeImageToBase64(f)));
                                 const updated = [...(editingProperty.gallery || []), ...newBase64s];
                                 setEditingProperty({ ...editingProperty, gallery: updated });
                                 triggerStatus('success', `${files.length} image(s) loaded! Click "Save All Blueprints & Property" below to apply changes.`);
                                 e.target.value = '';
                              }
                            }}
                ...
                <form onSubmit={handleSaveConcern} className="bg-[#161617] border border-white/10 p-8 rounded-sm space-y-6">
                  ...
                  <div className="space-y-4 p-5 border border-white/10 bg-black/30 rounded-sm">
                    ...
                    <div className="border border-dashed border-white/15 p-4 rounded-sm text-center bg-black/10">
                      <label className="block text-[10px] font-mono uppercase text-slate-400 mb-2">Upload multiple image files directly</label>
                      <input 
                        type="file" 
                        accept="image/*"
                        multiple
                        onChange={async (e) => {
                          ...
                          if (files.length > 0) {
                            triggerStatus('success', `Loading ${files.length} images...`);
                            const newBase64s = await Promise.all(files.map(f => resizeImageToBase64(f)));
                            const updatedGallery = [...(editingConcern.gallery || []), ...newBase64s];
                            setEditingConcern({ ...editingConcern, gallery: updatedGallery });
                            triggerStatus('success', `${files.length} image(s) added to gallery!`);
                            e.target.value = '';
                          }
                        }}

...
export default function PropertyDetailView({ property, onBack, onBookTour }: PropertyDetailViewProps) {
  ...
  const getAtGlanceValues = (prop: Property) => {
    ...
    return {
      address: prop.address || (isSkyline 
      ...
      floorsCount: `G+${prop.floorsCount - 1}`,
      aptPerFloor: prop.aptPerFloor || (isPlaza ? "Grade-A Workspaces" : "2"),
      aptSize: prop.sizeRange,
      beds: prop.beds ? String(prop.beds) : "3",
      baths: prop.baths ? String(prop.baths) : "3",
      launchDate: prop.launchDate || "October 2023",
      handoverDate: prop.handoverDate || (prop.completionYear ? `Completed ${prop.completionYear}` : "May 2028")
    };
  };
  ...
  return (
    <div className="bg-[#0B0B0C] min-h-screen text-slate-300 font-sans pb-28">
            ...
            {galleryImages.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-[#FF4A4F] text-white flex items-center justify-center border border-white/10 hover:border-transparent transition-all opacity-0 group-hover:opacity-100 cursor-pointer z-10"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button
                  onClick={handleNextImg}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-[#FF4A4F] text-white flex items-center justify-center border border-white/10 hover:border-transparent transition-all opacity-0 group-hover:opacity-100 cursor-pointer z-10"
                >
      ...
      <div className="max-w-[1360px] mx-auto px-6 sm:px-10 mt-16 space-y-16">
        ...
        {property.hideVirtualConfigurator !== true && property.floorLayouts && property.floorLayouts.length > 0 && (
          <div className="space-y-6 pt-12 border-t border-white/5">
            ...
            <div className="flex flex-wrap gap-2 pb-2">
              {property.floorLayouts.map((layout, idx) => {
            ...
            <div className="border border-white/10 bg-[#0E0E0F] rounded-lg p-6 flex flex-col justify-between space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
                <div className="space-y-1 text-left">
                  ...
                  <h4 className="font-serif text-2xl text-white font-medium uppercase tracking-tight">
                    {property.floorLayouts[activeFloorIndex].levelName}
                  </h4>
                </div>
                ...
                <div className="flex items-center gap-6 text-xs font-mono">
                  <div>
                    ...
                    <span className="text-[#FF4A4F] font-bold text-base">
                      {property.floorLayouts[activeFloorIndex].sizeSqft.toLocaleString()} SFT
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="text-white font-bold text-base">
                      {property.floorLayouts[activeFloorIndex].rooms.length} Spaces
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div 
                ...
                onClick={() => setLightboxImage(property.floorLayouts[activeFloorIndex]?.imageUrl || floorPlanImages[activeFloorIndex % floorPlanImages.length])}
              >
                {/* Compass Decor */}
                ...
                <img
                  src={property.floorLayouts[activeFloorIndex]?.imageUrl || floorPlanImages[activeFloorIndex % floorPlanImages.length]}
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden border border-white/10 rounded bg-black/50 group">
              <img
                ...
                loading="lazy"
                className={`w-full h-full filter brightness-75 group-hover:scale-102 transition-all duration-700 ease-out ${getFitClass(property.imageFit)} ${getPositionClass(property.imagePosition)}`}
              />
              {/* Elegant floating branding headers overlay */}
      ...
      <FloorPlanZoomModal
        ...
        title={`${property.title} - ${property.floorLayouts?.[activeFloorIndex]?.levelName || 'Floor Blueprint'}`}
        floorLayouts={property.floorLayouts}
        activeFloorIndex={activeFloorIndex}
        onSelectFloor={(idx) => {
          setActiveFloorIndex(idx);
          if (property.floorLayouts && property.floorLayouts[idx]) {
            setLightboxImage(property.floorLayouts[idx].imageUrl || floorPlanImages[idx % floorPlanImages.length]);
          }
        }}
      />

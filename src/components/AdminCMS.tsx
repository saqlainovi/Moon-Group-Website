  ...
  const handleSaveProperty = async (e: React.FormEvent) => {
    ...
    await runSaveWithFirebaseCrosscheck(
      ...
      async () => {
        ...
        if (siteSettings && (currentProp.id === 'haven-tower' || (currentProp.title && currentProp.title.toLowerCase().includes('haven tower')))) {
  ...
  const handleSaveSlide = async (e: React.FormEvent) => {
    ...
    await runSaveWithFirebaseCrosscheck(
      ...
      async () => {
        ...
        if (currentSlide.id === 'haven-tower' || (currentSlide.title && currentSlide.title.toLowerCase().includes('haven tower'))) {
          if (siteSettings) {
            const updatedSettings = { ...siteSettings, havenTowerBannerImage: currentSlide.imageUrl };
            setSiteSettings(updatedSettings);
            await saveCMSSiteSettings(updatedSettings).catch(() => {});
          }
          const havenInList = properties.find(p => p.id === 'haven-tower' || (p.title && p.title.toLowerCase().includes('haven tower')));
          if (havenInList) {

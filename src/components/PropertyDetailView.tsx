...
export default function PropertyDetailView({ property, onBack, onBookTour }: PropertyDetailViewProps) {
  ...
  const getAtGlanceValues = (prop: Property) => {
    ...
    const isSkyline = propId.includes('skyline');
    const isImperial = propId.includes('imperial');
    const isPlaza = propId.includes('corporate');
    const isHaven = propId.includes('haven') || propTitle.includes('haven') || propTitle.includes('heaven');

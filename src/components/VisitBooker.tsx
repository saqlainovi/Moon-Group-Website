...
export default function VisitBooker({ preSelectedProperty, onClearPreSelected, propertiesList }: VisitBookerProps) {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    const ticket: VisitBooking = {
      ...
      date: 'Flexible (Contact to Arrange)',
      timeSlot: '10:00 AM - 05:00 PM (Friday Off)',
            ...
            >
              ...
              <form onSubmit={handleBookingSubmit} className={`md:col-span-8 p-6 sm:p-8 flex flex-col justify-between border-l transition-all duration-500 ${
                ...
                <div className="space-y-6">
                  ...
                  {/* Step 2: Visiting Hours Info Box */}
                  <div>
                    <label className={`block text-[10px] uppercase tracking-widest font-bold mb-2.5 ${isDark ? 'text-gold-400' : 'text-stone-700'}`}>
                      2. Tour Timing & Availability
                    </label>
                    <div className={`p-4 rounded-md border transition-all duration-300 ${
                      isDark 
                        ? 'bg-luxury-beige/35 border-white/10 text-slate-200' 
                        : 'bg-white border-stone-300/60 text-stone-800'
                    }`}>
                      <div className="flex items-center gap-2 mb-2.5">
                        <Clock className="w-4 h-4 text-[#EE1B24]" />
                        <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#EE1B24]">
                          Official Visiting Hours
                        </span>
                      </div>
                      <p className="text-xs leading-relaxed font-sans font-medium">
                        আমাদের ভিজিটিং সময় প্রতিদিন সকাল <span className="font-bold text-[#EE1B24] font-mono text-sm">10:00 AM</span> থেকে বিকাল <span className="font-bold text-[#EE1B24] font-mono text-sm">5:00 PM</span> টা পর্যন্ত।
                      </p>
                      <p className="text-xs leading-relaxed font-sans font-semibold text-[#EE1B24] mt-1.5 flex items-center gap-1">
                        <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#EE1B24]" />
                        সাপ্তাহিক ছুটির দিন: প্রতি শুক্রবার সম্পূর্ণ বন্ধ (Friday Closed).
                      </p>

                    </div>
                  </div>

                  {/* Step 3: Profile Details Inputs */}
                  <div>

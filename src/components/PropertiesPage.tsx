...
export default function PropertiesPage({
  ...
  const havenProp = finalProperties.find(p => p.id === 'haven-tower' || (p.title && p.title.toLowerCase().includes('haven tower'))) || finalProperties[0];
  const havenBannerImage = siteSettings?.havenTowerBannerImage || havenProp?.imageUrl || 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=1200&q=80';
  ...
  const filteredProperties = finalProperties.filter((prop) => {
    ...
    const matchesSearch =
      !query ||
      (prop.title ? prop.title.toLowerCase().includes(query) : false) ||
      (prop.location ? prop.location.toLowerCase().includes(query) : false) ||
      (prop.area ? prop.area.toLowerCase().includes(query) : false);
              ...
              <div className={`border p-6 rounded-xl flex flex-col sm:flex-row gap-6 ${
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['all', 'ongoing', 'under-construction', 'upcoming', 'proposed', 'completed'].map((status) => (
                      <button
                        key={status}

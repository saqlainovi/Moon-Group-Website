...
const defaultSiteSettings: CMSSiteSettings = {
  ...
  havenTowerSizes: '1,850 Sft - 3,600 Sft (Single & Duplex)',
...
export async function seedCMSDatabaseIfNeeded() {
  ...
  try {
    // 1. Seed Properties (Seed missing items so all 13 projects exist)
    const propertiesSnap = await withTimeout(getDocs(collection(db, 'properties')), 4000, true);
    const existingPropertyIds = new Set(propertiesSnap.docs.map(d => d.id));
    for (const p of defaultProperties) {
      if (!existingPropertyIds.has(p.id)) {
        console.log(`Seeding Property: ${p.id}...`);
...
export async function getCMSSiteSettings(): Promise<CMSSiteSettings> {
  ...
  try {
    ...
    console.warn('Server API site settings fetch notice, trying Supabase direct fallback', err);
  }
  ...
  try {
    ...
    if (!error && row && row.value) {
      const merged = { ...defaultSiteSettings, ...row.value };
      await saveSiteSettingsToLocalStorage(merged);
      return await processObjectImagesForLoad(merged);
    }
  } catch (err) {
    console.warn('Direct Supabase site settings load exception:', err);
  }
...
export async function saveCMSSiteSettings(data: CMSSiteSettings): Promise<void> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      apiErrorMsg = `HTTP Error ${res.status}: ${await res.text()}`;
    }
  } catch (err: any) {
  ...
  try {
    const { error: upsertErr } = await supabase.from('cms_store').upsert({
      key: 'siteSettings',
      value: data,
      updated_at: new Date().toISOString()
    });
    if (upsertErr) {
      supabaseErrorMsg = upsertErr.message;
    } else {
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save Site Settings on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function getCMSProperties(): Promise<Property[]> {
  ...
  try {
    ...
    console.warn('Server API properties fetch notice, trying Supabase fallback', err);
  }
  ...
  try {
    ...
    if (!error && row && Array.isArray(row.value) && row.value.length > 0) {
      const resolvedDocs = await Promise.all(
        row.value.map((doc: any) => processObjectImagesForLoad(doc))
      );
      for (const p of resolvedDocs) {
        await savePropertyToLocalStorage(p);
      }
    ...
    console.warn('Direct Supabase properties load exception:', err);
  }
...
export async function saveCMSProperty(property: Property): Promise<void> {
  ...
  // Direct Supabase Fallback Write
  try {
    const { data: row, error: selectErr } = await supabase.from('cms_store').select('value').eq('key', 'properties').maybeSingle();
    if (selectErr) {
      supabaseErrorMsg = selectErr.message;
    } else {
      let currentList = row?.value;
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save property on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function deleteCMSProperty(id: string): Promise<void> {
  ...
  // Direct Supabase Fallback Delete
  try {
    const { data: row, error: selectErr } = await supabase.from('cms_store').select('value').eq('key', 'properties').maybeSingle();
    if (selectErr) {
      supabaseErrorMsg = selectErr.message;
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to delete property from both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function getCMSAboutUs(): Promise<CMSAboutUs> {
  ...
  try {
    ...
    console.warn('Server API about us fetch notice, trying Supabase fallback', err);
  }
  ...
  try {
    const { data: row, error } = await supabase
      ...
      .maybeSingle();
    if (!error && row && row.value) {
      const loaded = await processObjectImagesForLoad(row.value);
      const merged = { ...defaultAboutUs, ...loaded };
      await saveAboutUsToLocalStorage(merged);
      return merged;
    }
  } catch (err) {
    console.warn('Direct Supabase aboutUs load exception:', err);
  }
...
export async function saveCMSAboutUs(data: CMSAboutUs): Promise<void> {
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save About Us narrative on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function getCMSHeroSlides(): Promise<CMSHeroSlide[]> {
  ...
  try {
    ...
    console.warn('Server API hero slides fetch notice, trying Supabase fallback', err);
  }
  ...
  try {
    const { data: row, error } = await supabase
      ...
      .maybeSingle();
    if (!error && row && Array.isArray(row.value) && row.value.length > 0) {
      const filtered = row.value.filter((s: any) => s && s.id && !deletedIds.includes(s.id));
      const resolvedSlides = await Promise.all(
        filtered.map((slide: any) => processObjectImagesForLoad(slide))
      );
      for (const s of resolvedSlides) {
    ...
    console.warn('Direct Supabase hero slides load exception:', err);
  }
...
export async function saveCMSHeroSlide(slide: CMSHeroSlide): Promise<void> {
  ...
  // Direct Supabase Fallback Write
  try {
    const { data: row, error: selectErr } = await supabase.from('cms_store').select('value').eq('key', 'heroSlides').maybeSingle();
    if (selectErr) {
      supabaseErrorMsg = selectErr.message;
    } else {
      let currentList = row?.value;
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save slide on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function deleteCMSHeroSlide(id: string): Promise<void> {
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to delete slide from both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function getCMSGroupConcerns(): Promise<CMSGroupConcern[]> {
  ...
  try {
    ...
    if (json.success && Array.isArray(json.data) && json.data.length > 0) {
      ...
      const existingIds = new Set(resolvedList.map(item => item.id));
      const missingDefaults = defaultGroupConcerns.filter(item => !existingIds.has(item.id));
      const finalConcerns = [...resolvedList, ...missingDefaults];
      for (const c of finalConcerns) {
        await saveGroupConcernToLocalStorage(c);
      }
      return finalConcerns.sort((a, b) => a.num.localeCompare(b.num));
    }
  } catch (err) {
    console.warn('Server API group concerns fetch notice, trying Supabase fallback', err);
  }
  ...
  try {
    const { data: row, error } = await supabase
      .from('cms_store')
      .select('value')
      .eq('key', 'groupConcerns')
      .maybeSingle();
    if (!error && row && Array.isArray(row.value) && row.value.length > 0) {
      const resolvedList = await Promise.all(
        row.value.map((c: any) => processObjectImagesForLoad(c))
      );
      const existingIds = new Set(resolvedList.map(item => item.id));
      const missingDefaults = defaultGroupConcerns.filter(item => !existingIds.has(item.id));
      const finalConcerns = [...resolvedList, ...missingDefaults];
    ...
    console.warn('Direct Supabase groupConcerns load exception:', err);
  }
...
export async function saveCMSGroupConcern(concern: CMSGroupConcern): Promise<void> {
  ...
  // Direct Supabase Fallback Write
  try {
    const { data: row, error: selectErr } = await supabase.from('cms_store').select('value').eq('key', 'groupConcerns').maybeSingle();
    if (selectErr) {
      supabaseErrorMsg = selectErr.message;
    } else {
      let currentList = row?.value;
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save sister concern on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function deleteCMSGroupConcern(id: string): Promise<void> {
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to delete sister concern from both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function getCMSTestimonials(): Promise<CMSTestimonial[]> {
  ...
  try {
    ...
    console.warn('Server API testimonials fetch notice, trying Supabase fallback:', err);
  }
  ...
  try {
    ...
    if (!error && row && Array.isArray(row.value)) {
      const resolvedList = await Promise.all(
        row.value.map((t: any) => processObjectImagesForLoad(t))
      );
      for (const t of resolvedList) {
        await saveTestimonialToLocalStorage(t);
      }
    ...
    console.warn('Direct Supabase testimonials load exception:', err);
  }
...
export async function saveCMSTestimonial(testimonial: CMSTestimonial): Promise<void> {
  ...
  // Direct Supabase Fallback Write
  try {
    const { data: row, error: selectErr } = await supabase.from('cms_store').select('value').eq('key', 'testimonials').maybeSingle();
    if (selectErr) {
      supabaseErrorMsg = selectErr.message;
    } else {
      let currentList = row?.value;
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to save customer review on both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
...
export async function deleteCMSTestimonial(id: string): Promise<void> {
  ...
  if (!apiSucceeded && !supabaseSucceeded) {
    throw new Error(`Failed to delete customer review from both API server and cloud database.\nAPI Details: ${apiErrorMsg}\nSupabase Details: ${supabaseErrorMsg}`);
  }
